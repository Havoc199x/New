...
// SQL query to fetch student data including major_offense and minor_offense
$sql = "SELECT u.user_id, u.user_name, s.csection, c.course_name, st.minor_offense, st.major_offense
        FROM user u 
        JOIN section s ON u.user_id = s.user_id 
        JOIN course c ON u.user_id = c.user_id 
        JOIN status st ON u.user_id = st.user_id
        WHERE u.role = 'student'";

$result = $conn->query($sql);

// Display the dynamic table with fetched data
...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo '<td>' . $row['csection'] . '</td>';
        echo '<td>' . $row['course_name'] . '</td>';
        echo '<td class="minor">' . $row['minor_offense'] . '</td>';
        echo '<td class="major">' . $row['major_offense'] . '</td>';
        echo '<td><button class="clear-btn" data-user-id="' . $row['user_id'] . '" style="cursor: pointer; background-color: green; border: none; color: white; padding: 4px; border-radius: 4px;">Clear</button></td>';
        echo '</tr>';    
    }
} else {
                ...
                .then(data => {
                    ...
                    data.messages.forEach(message => {
                        ...
                        if (message.file_type && message.file_type.trim() !== '') {
                            ...
                            imageElement.src = 'http://localhost:8080/DisciplinaryOffice/test2.jpg'; // Assuming file_type holds the image URL
                        } else {
                        ...
                        viewButton.addEventListener('click', function() {
                            ...
                            window.open('http://localhost:8080/DisciplinaryOffice/PersonCompleteDetails.php');
                        });
